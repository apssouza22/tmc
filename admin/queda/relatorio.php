	...
	<form method="post" name="form_ins" id="form_ins" action="<?php echo $_SERVER['PHP_SELF']; ?>" onsubmit="return checar(this)">
		<div class="caixa">
			...
			<div class="field  half">
				...
				<input type="date" class="datetimepicker " value="<?=$_POST['inicio']?>" name="inicio"></input>
			</div>
			<div class="field half">
				<label><span>Fim:</span></label>
				<input type="date" value="<?= isset($_POST['fim']) ? $_POST['fim'] : date('d/m/Y') ?>" class="datetimepicker " name="fim"></input>
			</div>

			<div class="field full controles">
				<a href="#" onclick="$('#form_ins').submit();return false;" class="bt_padrao ui-state-default ui-corner-all"><span class="ui-icon ui-icon-disk"></span>Buscar</a>
				<a href="#" onclick="$('#form_ins').submit();return false;" class="bt_padrao ui-state-default ui-corner-all"><span class="ui-icon ui-icon-disk"></span>Exportar</a>
